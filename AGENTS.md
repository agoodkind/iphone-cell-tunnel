...
`docs/machine.md` covers running the app in a disposable tart machine: which image
carries Xcode, how to fetch one quickly, the three approvals a tunnel needs before it
carries traffic, registering the packet tunnel extension, and driving the machine's
screen with idb.
...
- **Commands:** `make help` lists engine targets from swift-makefile, then the project targets declared under `help::` in the Makefile (printf only, no CellTunnelDev build). Named Make targets (`build-mac`, `relay-up`, ...) route into that tool. Start there.
...
- **`celltunnelctl` is the agent client; query it for the current command set.** Use the `CellTunnelDev` `relay-*` commands (`relay-up --config <path>`, `relay-status`, `relay-reload --config <path>`, `relay-down`) for tunnel operations, which poll the agent until it reports the outcome rather than reading one snapshot and guessing. `celltunnelctl --help` lists the raw client commands, and the agent-owned config library lives behind `celltunnelctl configs`.
...
- **Builds run in the foreground** so their output is visible. Build the iPhone target before `make iphone-install`, which deploys a prebuilt bundle. `make install-mac` builds the Mac agent itself before installing it.
...
`README.md` is the user-facing build, install, and run quickstart, and `make help` lists every target. Use the named build targets (`make build-mac`, `make build-iphone`, and the rest) for normal platform builds. `make build` and `make build-all` compile every platform. CI builds every platform in one Verify job, signs each product, and verifies each runnable product's signature. `make install-mac` builds the Mac agent, copies it to `/Applications/CellTunnel/CellTunnelAgent.app`, then always restarts the launchd service so the freshly built binary is the one running. `make iphone-install` deploys a prebuilt bundle, so build the iPhone target first.
...
1. Build the iPhone before installing it. `make iphone-install` installs a prebuilt bundle and does not build it, so after `make clean` or any source change build the iPhone target first. `make install-mac` builds the Mac agent itself, so it needs no separate build step:
   - macOS: `make install-mac CONFIG=Debug`.
   - iPhone: `make build-iphone CONFIG=Debug`, then `make iphone-install CONFIG=Debug`.
...
3. Select the iPhone. The agent hosts the link, the extensions dial it, and the agent publishes the record the iPhone browses for as soon as it starts, without waiting for a client. On a fresh machine that record does not publish until local network access is granted, which `docs/machine.md` covers. On a cold agent with no phone selected, `relay-up` stores the config in the agent's library, marks it active, and then fails with `relaySelectionRequired`, which is the expected first result. Wait for `celltunnelctl peers` to list the phone, then run `celltunnelctl select <n>` to make it the egress peer.
4. Bring up the Mac tunnel: `make relay-up WG_CONFIG=<path>`. With a peer selected it returns `running=true` with `routes=not-installed`, since the Mac VPN connects before the iPhone link is up. The relay is explicit: the agent runs until the login session ends and never stops itself, and it does not auto-restore a relay at boot, so a hosted relay stays up until something stops it. If `relay-status` shows `peers=0`/`links=0` and `mac-logs` shows no control-listener line, the relay is down, so bring it back with this step rather than assuming a torn-down session is reusable. When the iPhone extension dials in, the agent signals the extension and `relay-status` flips to `routes=installed`. Routes track the link, so a test before `routes=installed` measures nothing through the tunnel. Link failover between interfaces during a live relay is automatic and traffic-driven, so a single interface dropping does not need a restart.
...
- `RouteGate` also gates DNS by the link state. The config's `DNS =` servers are published as the system resolver (with `matchDomains` covering all queries) only while the iPhone link is up, and withdrawn when it drops, so an all-traffic config resolves names over the tunnel and reverts to the Mac's own resolver on link loss. When verifying an all-traffic config, check `scutil --dns` shows the tunnel resolver while the link is up and the normal resolver after a link drop or `relay-down`. A tunnel resolver that is reachable only over the tunnel is the reason DNS must be gated, not static.
