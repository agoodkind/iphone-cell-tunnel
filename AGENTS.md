...
- **Commands:** `make help` lists every build, lint, test, format, and install target with a one-line description. Engine targets come from swift-makefile; project targets append via `help::` which runs CellTunnelDev `make-help`. The Makefile only routes; help text lives in the tool. Start there.
