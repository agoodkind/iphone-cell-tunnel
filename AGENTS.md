...
- **Commands:** `make help` lists engine targets from swift-makefile, then appends CellTunnelDev's own help via `help::`. Project goals use `make ct/<args...>` as a passthrough to that tool (same argv as `swift Tools/cell-tunnel-dev.swift <args...>`). Start there.
...
1. Build before install. `make install-mac` and `make iphone-install` install a prebuilt bundle and do not build it. After `make clean` or any source change, build the target first:
   - macOS: `make ct/build/mac`, then `make ct/install-mac`.
   - iPhone: `make ct/build/iphone-device`, then `make ct/activate/iphone`.
...
3. Bring up the Mac tunnel: `make ct/relay-up ARGS='--config <path>'`. It returns `running=true` with `routes=not-installed`: the Mac VPN is connected immediately with no iPhone present. The agent hosts the link and the extensions dial it, so no relay selection is needed.
