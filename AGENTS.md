...
6. Probe reachability per protocol, not just speed, and confirm each crossed the relay. The relay carries any inner protocol opaquely, and a destination may answer one protocol and not another, so test the one you mean and read the `relay-status` counter delta around it (`mac_datagrams_from_server` and `mac_bytes_in` for replies received, `mac_datagrams_to_server` and `mac_bytes_out` for traffic sent).
   - Egress identity, v6 and v4: `curl -6 https://ident.me` and `curl -4 https://ident.me`. The host `ident.me` (`2a01:4f9:c012:8091::1`) is in `AllowedIPs`, so over the tunnel these return the WireGuard server's address rather than the Mac's native ISP address, which proves the traffic egresses through the relay and not the local link. Compare against the same `curl` with the tunnel down to see the native address.
   ...
   - TCP: `curl -6 https://[2600:1901:0:b2bd::]/` or another routed host. A TLS certificate error means the TCP connection completed, so the path works even when ICMP to the same host does not.
