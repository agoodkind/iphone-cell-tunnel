...
3. Start the pairing listener and select the iPhone. The agent hosts the link and the extensions dial it, but nothing advertises until a client asks for the listener. Opening the Mac Catalyst app starts it, and so does `make relay-up WG_CONFIG=<path>`, which starts the listener before it checks for a peer. On a cold agent with no phone selected, `relay-up` stores the config in the agent's library, marks it active, and then fails with `relaySelectionRequired`, which is the expected first result. Wait for `celltunnelctl peers` to list the phone, then run `celltunnelctl select <n>` to make it the egress peer.
