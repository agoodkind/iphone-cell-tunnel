...
/// tunnel cannot run. `PhoneRelayBackend` delegates to it while running in the
/// simulator; off the simulator it is held but never started. It does not
/// fabricate a routing session. Instead it exercises the real control path through
/// the shared `CellTunnelCore` primitives: it browses for the Mac agent's control
/// service, dials it with the shared `RelayControlFramer`, decodes the agent's
/// `setServerEndpoint` with the shared `RelayControlMessageCodec`, and acknowledges
/// it. A reached agent maps to passthrough, since the simulator can establish the
/// link but cannot capture or route device packets. Counters stay zero because
...
final class SimulatorRelayBackend: RelayControlBackend {
    ...
    // Routing needs the packet tunnel the simulator cannot run, so the choice is
    // recorded only as a log and the state stays passthrough.
    ...
    // The agent sends `setServerEndpoint` on accept and waits for an acknowledge.
    // Receiving and acknowledging it through the shared codec proves the control
    // link end to end.
    ...
    // Reports the relay server address the agent named, matched to the requested
    // family, since it is the public identity device traffic would egress through.
    // The agent declares the family on the endpoint.
