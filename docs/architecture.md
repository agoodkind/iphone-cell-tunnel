...
- Mac agent (`CellTunnelAgent`): hosts the relay data listener, bridging datagrams between the Mac extension over loopback and the iPhone over the local link. It exits when idle, but checks for a live relay before exiting so it never kills its own bridge mid-session, and lengthens the countdown while advertising so a phone still has time to connect.
...
- The agent's idle countdown always runs, and what the agent is doing decides only how long it is. Hosting a relay or advertising for a phone lengthens it; with nothing in flight the short countdown applies. Whether the agent actually exits is decided when the countdown fires, by asking at that moment whether a relay is hosted. A relay with a live phone link restarts the countdown instead of exiting, because the agent owns that bridge in memory and the iPhone's UDP data link surfaces no drop on its own. A hosted relay whose link is momentarily down, which is what a failover between interfaces looks like, gets a bounded number of short waits before the agent decides the phone has gone. Deciding at the moment of exit rather than trusting the state recorded when a start began matters because a bridge can fail afterwards with nothing to announce it, which would otherwise leave the agent resident with nothing running.
