...
- Mac agent (`CellTunnelAgent`): hosts the relay data listener, bridging datagrams between the Mac extension over loopback and the iPhone over the local link. It runs until the login session ends and never stops itself, because it owns the relay bridge, the Bonjour records the iPhone browses for, and the live link set, none of which survive the process. Its iPhone-facing control listener starts only on a client request, either a pairing request, a tunnel start, or turning routing on, so after a reboot with no app open and no command run the Mac advertises nothing and an iPhone browsing for it finds nothing.
...
The views bind to one observable controller, `RelayController`, which holds a `RelayControlBackend` and never branches on platform. `RelayController` owns the status poll: it samples its backend once a second in a loop it starts and cancels, and applies each reading to the views. Two backends sit behind it. `PhoneRelayBackend` (iPhone) owns the tunnel manager, the on-demand rule, the device-name publish, and answers each sample over the provider message channel. `AgentRelayBackend` (Mac) reads the agent's status and maps it onto the same reading the views render.
...
- `RouteGate` is also the sole authority over DNS. The config's `DNS =` servers are program-owned the same way routes are: while the iPhone link is up the gate publishes them as the system resolver with `matchDomains` covering all queries, so an all-traffic config resolves names over the tunnel and overrides any other system resolver, and while the link is down the gate withdraws DNS so the Mac falls back to its own resolver rather than a tunnel resolver it can no longer reach. A scoped config with no `DNS =` line publishes no tunnel DNS. WireGuardKit's own DNS settings are left unused so DNS stays gated.
...
- The agent runs until the login session ends and never stops itself. It holds the relay bridge, the Bonjour records the iPhone browses for, and the live link set in memory, and neither the iPhone nor the packet-tunnel extension can start it, so a self-imposed exit would end sessions that nothing present could restore. Only the Mac app and the command-line tool reach the agent, and the two signal handlers for `SIGINT` and `SIGTERM` are its only exits, each tearing the relay down first so the packet tunnel is never left holding routes to a bridge that is gone.
- Nothing starts the agent's iPhone-facing control listener except a client request. Its launchd job registers a Mach service, so launchd starts the process on demand, and the process serves the XPC control protocol from startup, but it publishes no Bonjour record until a pairing request, a tunnel start, or a routing-enable request asks for the listener. A Mac that has just booted with no app open and no command run therefore advertises nothing, and an iPhone browsing for it finds nothing.
