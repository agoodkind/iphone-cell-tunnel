...
enum WireGuardConfigParser {
    static func load(from url: URL) throws -> WireGuardClientConfig {
        ...
        do {
            ...
            logger.error(
                """
                wireguard config read failed path=\(url.path, privacy: .public) \
    ...
    static func parse(_ text: String) throws -> WireGuardClientConfig {
        ...
        for rawLine in text.split(omittingEmptySubsequences: false, whereSeparator: { $0 == "\n" || $0 == "\r" }) {
            ...
            let value = stripped[stripped.index(after: separator)...].trimmingCharacters(in: .whitespaces)
    ...
    private static func parsePrefix(_ value: String) throws -> AddressPrefix {
        ...
        let defaultLength = family == .ipv4 ? 32 : 128
        ...
        let bounds = family == .ipv4 ? 0...32 : 0...128
